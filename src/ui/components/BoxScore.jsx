...
function TeamSummaryTable({ homeStats, awayStats, homeAbbr, awayAbbr }) {
  const rows = [
    ...
    {
      label: "Pass Att/Comp",
      h: `${homeStats.passComp}/${homeStats.passAtt}`,
      a: `${awayStats.passComp}/${awayStats.passAtt}`,
    },
  ];
  ...
  Rushing: {
    ...
    columns: [
      ...
      { key: "rushTD", label: "TD", minWidth: 32 },
      {
        ...
        format: (r) =>
          r.stats?.rushAtt
            ? (r.stats.rushYd / r.stats.rushAtt).toFixed(1)
            : "—",
      },
...
export default function BoxScore({ gameId, actions, onClose }) {
  ...
  const homeRows = useMemo(
    () => filterAndSortPlayers(game?.stats?.home, activeTab),
    [game, activeTab],
  );
  const awayRows = useMemo(
    () => filterAndSortPlayers(game?.stats?.away, activeTab),
    [game, activeTab],
  );
      ...
      >
        ...
        <ScrollArea style={{ flex: 1 }}>
          ...
          {!loading && error && (
            ...
            >
              <div
                style={{ fontSize: "1.5rem", marginBottom: "var(--space-3)" }}
              >
                ⚠
              </div>
