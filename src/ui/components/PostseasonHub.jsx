...
function MatchupCard({ game, teams, userTeamId, seedByTeam }) {
  ...
  return (
    <div style={{
      ...
      padding: 'var(--space-3)',
      minWidth: 220,
      boxShadow: isUserGame ? '0 0 12px rgba(10,132,255,0.2)' : 'var(--shadow-sm)',
    }}>
      {/* Away team row */}
      <div style={{
        ...
        {awaySeed && (
          <span style={{
            ...
            background: teamColor(away.abbr), color: '#fff',
        ...
        <span style={{
          flex: 1, fontWeight: awayWon ? 800 : 600,
      ...
      <div style={{
        ...
        {homeSeed && (
          <span style={{
            width: 20, height: 20, borderRadius: '50%',
            background: teamColor(home.abbr), color: '#fff',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontWeight: 900, fontSize: 10, flexShrink: 0,
          }}>
            {homeSeed}
        ...
        <span style={{
          flex: 1, fontWeight: homeWon ? 800 : 600,
          fontSize: 'var(--text-sm)', color: 'var(--text)',
        }}>
          {home.abbr}
        </span>
...
export default function PostseasonHub({ league }) {
  const { schedule, teams, playoffSeeds, userTeamId, week, championTeamId } = league;
  ...
  if (!playoffSeeds) {
    return (
      <div style={{ textAlign: 'center', padding: 'var(--space-8)', color: 'var(--text-muted)' }}>
        The postseason hasn't started yet. Advance through the regular season to see the bracket.
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        padding: 'var(--space-4)',
        background: 'linear-gradient(135deg, rgba(255,215,0,0.08), rgba(192,192,192,0.08))',
        ...
        {userInPlayoffs && (
          <div style={{
            fontSize: 'var(--text-sm)', color: 'var(--accent)', fontWeight: 700,
          }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        {playoffWeeks.map(weekData => {
          const roundLabel = ROUND_LABELS[weekData.week] || `Week ${weekData.week}`;
