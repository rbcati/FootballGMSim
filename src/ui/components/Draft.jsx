    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          color: grade.color, marginBottom: 'var(--space-2)',
          letterSpacing: '2px',
        }}>
          GRADE: {grade.grade}
        </div>
        <div style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 'var(--space-1)' }}>
          {pick.playerName}
        </div>
...
function DraftTicker({ completedPicks }) {
  ...
  return (
    <div className="draft-ticker" style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          flexShrink: 0, marginRight: 'var(--space-2)',
        }}>
          LATEST
        </span>
        <div style={{
...
function TradeUpModal({ currentPick, league, actions, onClose, onTradeComplete }) {
      ...
      >
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
              Currently owned by {currentPick?.teamAbbr} · Round {currentPick?.round}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {result && (
          <div style={{
            ...
            background: result.accepted ? 'rgba(52,199,89,0.1)' : 'rgba(255,69,58,0.1)',
            marginBottom: 'var(--space-4)',
        ...
        <div style={{ marginBottom: 'var(--space-3)' }}>
          <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 'var(--space-2)', textTransform: 'uppercase' }}>
            Offer Draft Picks
          </div>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-1)', flexWrap: 'wrap', marginTop: 'var(--space-2)' }}>
              {myPicks.map(pk => (
                <span key={pk.id} style={{
                  ...
                  {pk.year} R{pk.round}
                  <button onClick={() => removePick(pk.id)} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', padding: 0, fontSize: 11 }}>x</button>
                </span>
              ))}
            </div>
          )}
        ...
        <div style={{ marginBottom: 'var(--space-3)' }}>
          ...
          <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid var(--hairline)', borderRadius: 'var(--radius-md)' }}>
            ...
            {myRoster.sort((a, b) => (b.ovr ?? 0) - (a.ovr ?? 0)).map(p => (
              <label key={p.id} style={{
                display: 'flex', alignItems: 'center', gap: 'var(--space-2)',
                padding: 'var(--space-1) var(--space-2)',
                borderBottom: '1px solid var(--hairline)',
                cursor: 'pointer', fontSize: 'var(--text-xs)',
                background: offering.has(p.id) ? 'var(--accent)11' : 'transparent',
              }}>
                <input type="checkbox" checked={offering.has(p.id)} onChange={() => togglePlayer(p.id)}
                  style={{ accentColor: 'var(--accent)', width: 12, height: 12 }} />
                <span style={{
                  padding: '0 3px', borderRadius: 'var(--radius-pill)',
                  background: `${ovrColor(p.ovr)}22`, color: ovrColor(p.ovr),
                  fontWeight: 700, fontSize: 10,
                }}>{p.ovr}</span>
...
function DraftBoard({ draftState, userTeamId, onSimToMyPick, onDraftPlayer, onPlayerClick, simming, league, actions }) {
  ...
  const handleDraftPlayer = useCallback(async (playerId) => {
    ...
    try {
      ...
      if (res?.payload) {
        ...
        const lastUserPick = [...picks].reverse().find(
