...
function getPrimaryAction({ weekly, latestGameId, nextGame, busy, simulating }) {
  ...
  if (nextGame) {
    return {
      label: "Set Lineup",
      cta: "Set Lineup",
      detail: `Week ${nextGame.week} ${nextGame.isHome ? "vs" : "@"} ${nextGame.opp?.name ?? "TBD"} is next.`,
      tab: "Depth Chart",
      onClick: "navigate",
      disabled: false,
    };
    ...
    <div className="app-screen-stack franchise-hq">
      <section className="card franchise-hq-hero">
        <div className="franchise-hq-hero__top">
          <div>
            ...
            <p className="franchise-hq-hero__subtitle">{primaryAction.detail}</p>
          </div>
      ...
      <section className="card franchise-hq-flow">
        ...
        <div className="franchise-hq-flow__col">
          ...
          <p>{weekly?.phasePriority ?? "Keep momentum and execute your plan."}</p>
        </div>
      </section>
