...
function ValueBar({ myValue, theirValue }) {
  ...
  const fairnessColor =
    Math.abs(diff) < total * 0.15
      ? "var(--success)"
      : diff > 0
        ? "var(--accent)" // we receive more
...
function PickSelector({ side, picks, onChange }) {
  ...
  const addPick = () => {
    onChange(side, {
      ...
      round,
      year,
      id: `${side}_${round}_${year}_${Date.now()}`,
    });
  };
...
function TradeBlockSummary({
  ...
  const offeredPlayers = [...offering]
    .map((id) => myRoster.find((p) => p.id === id))
    .filter(Boolean);
  const receivedPlayers = [...receiving]
      ...
      >
        ...
        <div>
          ...
          >
            ...
            {myPicks.map((pk) => (
              <PickPill key={pk.id} pk={pk} />
            ))}
            {offeredPlayers.length === 0 && myPicks.length === 0 && (
              <EmptyNote />
            )}
          </div>
        </div>
        ...
        >
          <span
            style={{ fontSize: "var(--text-xl)", color: "var(--text-subtle)" }}
          >
            ⇄
          </span>
        </div>
        ...
        <div>
          ...
          >
            ...
            {receivedPlayers.length === 0 && theirPicks.length === 0 && (
              <EmptyNote />
            )}
          </div>
        </div>
...
export default function TradeCenter({ league, actions, onPlayerSelect }) {
  ...
  const fetchRosters = useCallback(
    async (tId) => {
      ...
      try {
        ...
        if (myResp?.payload) {
          setMyRoster(myResp.payload.players ?? []);
          setMyTeam(myResp.payload.team);
        }
        if (theirResp?.payload) {
          setTheirRoster(theirResp.payload.players ?? []);
          setTheirTeam(theirResp.payload.team);
        }
      } catch (e) {
  ...
  const liveTheirTeam = useMemo(
    () =>
      targetId != null
        ? (league?.teams?.find((t) => t.id === targetId) ?? theirTeam)
        : theirTeam,
  ...
  const myOfferValue = useMemo(() => {
    const pv = [...offering].reduce(
      (s, id) => s + playerTradeValue(myRoster.find((p) => p.id === id)),
      0,
    );
  ...
  const theirOfferValue = useMemo(() => {
    const pv = [...receiving].reduce(
      (s, id) => s + playerTradeValue(theirRoster.find((p) => p.id === id)),
      0,
    );
    const kv = theirPicks.reduce(
      (s, pk) => s + (PICK_VALUES[pk.round] ?? 10),
      0,
    );
    return pv + kv;
  ...
  const myCapAfter = useMemo(() => {
    ...
    const freed = [...offering].reduce(
      (s, id) =>
        s + (myRoster.find((p) => p.id === id)?.contract?.baseAnnual ?? 0),
      0,
    );
    const absorbed = [...receiving].reduce(
      (s, id) =>
        s + (theirRoster.find((p) => p.id === id)?.contract?.baseAnnual ?? 0),
      0,
    );
  ...
  const theirCapAfter = useMemo(() => {
    ...
    const freed = [...receiving].reduce(
      (s, id) =>
        s + (theirRoster.find((p) => p.id === id)?.contract?.baseAnnual ?? 0),
      0,
    );
    const absorbed = [...offering].reduce(
      (s, id) =>
        s + (myRoster.find((p) => p.id === id)?.contract?.baseAnnual ?? 0),
      0,
    );
  ...
  const toggleOffering = (id, chk) =>
    setOffering((prev) => {
      const s = new Set(prev);
      chk ? s.add(id) : s.delete(id);
      return s;
    });
  const toggleReceiving = (id, chk) =>
    setReceiving((prev) => {
      const s = new Set(prev);
      chk ? s.add(id) : s.delete(id);
      return s;
  ...
  const handlePropose = async () => {
    ...
    try {
      ...
      setTradeResult({
        accepted: false,
        reason: "Error communicating with the game engine.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const myFiltered = useMemo(
    () =>
      filterRoster(myRoster, searchMy).sort(
        (a, b) => (b.ovr ?? 0) - (a.ovr ?? 0),
      ),
    [myRoster, searchMy],
  );
  const theirFiltered = useMemo(
    () =>
      filterRoster(theirRoster, searchTheir).sort(
        (a, b) => (b.ovr ?? 0) - (a.ovr ?? 0),
      ),
    [theirRoster, searchTheir],
  );
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {offering.size} player{offering.size !== 1 ? "s" : ""} +{" "}
                    ...
                    {offering.size > 0 && (
                      <span
                        style={{
                          marginLeft: 6,
                          color:
                            myCapAfter < 0 ? "var(--danger)" : "var(--success)",
                        }}
                      >
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {receiving.size} player{receiving.size !== 1 ? "s" : ""} +{" "}
                    {theirPicks.length} pick{theirPicks.length !== 1 ? "s" : ""}{" "}
                    selected
                    {receiving.size > 0 && (
                      <span
                        style={{
