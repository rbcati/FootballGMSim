  ...
  return (
    ...
    >
      ...
      <TeamBadge
        abbr={awayTeam?.abbr}
        size={36}
        highlight={awayTeam?.id === userTeamId}
      />
      ...
      <TeamBadge
        abbr={homeTeam?.abbr}
        size={36}
        highlight={homeTeam?.id === userTeamId}
      />
    </div>
  );
}
...
function generateFakePlays(homeAbbr, awayAbbr, count = 8) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const tpl =
      PLAY_TEMPLATES[Math.floor(Math.random() * PLAY_TEMPLATES.length)];
...
export default function LiveGameViewer({
  ...
  useEffect(() => {
    ...
    const game =
      currentWeekGames[Math.floor(Math.random() * currentWeekGames.length)];
          ...
          >
            {isFinished
              ? lastResults.map((r, i) => {
                  const home = {
                    id: r.homeId,
                    abbr: r.homeName?.slice(0, 3) ?? "???",
                    ...teamById[r.homeId],
                  };
                  const away = {
                    id: r.awayId,
                    abbr: r.awayName?.slice(0, 3) ?? "???",
                    ...teamById[r.awayId],
                  };
                  return (
                    <MatchupCard
            ...
            {plays.map((play, i) => (
              <div
                ...
                style={{
                  ...
                  animation:
                    i === plays.length - 1 ? "fadeSlideIn 0.25s ease" : "none",
                }}
              >
                {play}
              </div>
