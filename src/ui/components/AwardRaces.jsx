...
function CandidateRow({ candidate, rank, onPlayerSelect, showConf = false }) {
  ...
  return (
    <div
      onClick={() =>
        candidate.playerId != null && onPlayerSelect?.(candidate.playerId)
      }
      style={{
        display: "flex",
        alignItems: "center",
    ...
    >
      ...
      >
        {(keyStats ?? []).slice(0, 3).map((ks, i) => (
          <div
            key={i}
            style={{ textAlign: "center", minWidth: 36, whiteSpace: "nowrap" }}
          >
            <div
...
function MVPTab({ award, onPlayerSelect }) {
  return (
    <div>
      ...
      >
        League-wide ranking. QBs on winning teams carry a significant edge, but
        elite defensive performances can contend.
      </p>
      <AwardCard
...
function OPOYTab({ award, onPlayerSelect }) {
  return (
    ...
    >
      ...
      >
        Best offensive player in each conference. QBs, RBs, WRs, and TEs all
        compete.
      </p>
      <ConferenceSplit
...
function DPOYTab({ award, onPlayerSelect }) {
  return (
    ...
    >
      ...
      >
        Best defensive player in each conference. Pass rushers, shutdown
        corners, and tackling machines all contend.
      </p>
      <ConferenceSplit
...
function AllProSlot({ player, onPlayerSelect }) {
  ...
  return (
    <div
      onClick={() =>
        player.playerId != null && onPlayerSelect?.(player.playerId)
      }
      style={{
        display: "flex",
        alignItems: "center",
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: "var(--space-6)" }}>
        ...
        >
          <h3
            style={{ margin: 0, fontSize: "var(--text-xl)", fontWeight: 800 }}
          >
            Award Races
          </h3>
          {!loading && data && (
        ...
        {!loading && !error && !isOffseason && (
          ...
          >
            Rankings are updated each time you open this tab and reflect live
            stats through the latest week played.
          </p>
        )}
      </div>
      ...
      {!loading && !error && !data && (
        ...
        >
          ...
          <div>
            No award data available — play through at least a couple of weeks to
            see the races begin.
          </div>
        </div>
      )}
      ...
      {!loading && !error && data && (
        <>
          ...
          {/* ── Tab content ── */}
          {activeTab === "mvp" && (
            <MVPTab award={awards} onPlayerSelect={onPlayerSelect} />
          )}
          {activeTab === "opoy" && (
            <OPOYTab award={awards} onPlayerSelect={onPlayerSelect} />
          )}
          {activeTab === "dpoy" && (
            <DPOYTab award={awards} onPlayerSelect={onPlayerSelect} />
          )}
          {activeTab === "oroy" && (
            <OROYTab award={awards} onPlayerSelect={onPlayerSelect} />
          )}
          {activeTab === "droy" && (
            <DROYTab award={awards} onPlayerSelect={onPlayerSelect} />
          )}
          {activeTab === "allpro" && (
            <AllProTab allPro={allPro} onPlayerSelect={onPlayerSelect} />
          )}
        </>
      )}
    </div>
