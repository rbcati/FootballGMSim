...
function groupByObjectLoop() {
  let sum = 0;
  const winnersByConf = Object.create(null);
  for (let i = 0; i < winnersFlat.length; i++) {
    const tid = winnersFlat[i];
    const cid = getConf(tid);
    if (winnersByConf[cid] === undefined) {
      winnersByConf[cid] = [];
    }
  ...
  for (const confId of confs) {
    const survivors = winnersByConf[confId];
...
let startGroupObjLoop = performance.now();
for (let i = 0; i < ITERATIONS; i++) groupByObjectLoop();
let endGroupObjLoop = performance.now();

console.log(`Baseline: ${endOld - startOld} ms`);
console.log(`Optimized (groupBy Object loop): ${endGroupObjLoop - startGroupObjLoop} ms`);
