...
function optimizedOnePassMap() {
  let sum = 0;
  // Map all winners to objects in a single pass
  const mappedWinners = winnersFlat.map(tid => ({
    teamId: tid,
    seed: getSeed(tid),
    conf: getConf(tid)
  }));
...
for (let i = 0; i < ITERATIONS; i++) optimizedOnePassMap();
let endOpt = performance.now();

console.log(`Baseline: ${endOld - startOld} ms`);
console.log(`Optimized (onePassMap): ${endOpt - startOpt} ms`);
