...
describe('load pipeline regression guards', () => {
  ...
  it('guards weekly and offseason evolution from duplicate application', () => {
    const workerSource = readFileSync(resolve(process.cwd(), 'src/worker/worker.js'), 'utf8');
    expect(workerSource.includes('model.lastEvolutionStamp === stamp')).toBe(true);
    expect(workerSource.includes('developmentModel.lastOffseasonEvolutionKey === offseasonKey')).toBe(true);
    expect(workerSource.includes('processOffseasonEvolution({')).toBe(true);
    expect(workerSource.includes('lastOffseasonEvolutionKey: offseasonKey')).toBe(true);
  });
});
