...
import { makeCoach, generateInitialStaff, getMedicalStaffInjuryMod } from '../core/coach-system.js';
...
import { processWeeklyEvolution, processOffseasonEvolution } from '../core/progression/evolutionEngine.ts';
...
function buildViewState() {
  ...
  return {
    activeLeagueId: getActiveLeagueId() ?? null,
...
function buildRosterView(teamId) {
  ...
  return players.map(p => {
    ...
    return {
        ...
        wearAndTear: Number(p?.wearAndTear ?? 0),
        growthHistory: Array.isArray(p?.growthHistory) ? p.growthHistory.slice(-8) : [],
        developmentContext: p?.developmentContext ?? null,
        contract,
        traits:   p.traits ?? [],
        schemeFit: fit,
        morale:    morale
...
function hydratePlayerDevelopmentFields(player = {}) {
  ...
  return {
    ...
    wearAndTear: Math.max(0, Math.min(100, Number(player?.wearAndTear ?? 0) || 0)),
...
function normalizeWeeklyDevelopmentMeta(metaObj = {}) {
  return {
    ...
    lastOffseasonEvolutionKey: metaObj?.developmentModel?.lastOffseasonEvolutionKey ?? null,
  };
}
...
function buildTeamDevelopmentFocusMap(metaObj = ensureDynastyMeta(cache.getMeta())) {
  ...
  for (const team of cache.getAllTeams()) {
    ...
    const staffBonuses = computeStaffTeamBonuses(team, {
      staffImpactStrength: Number(metaObj?.settings?.staffImpactStrength ?? 50),
      year: Number(metaObj?.year ?? 2025),
    });
    const medicalMod = getMedicalStaffInjuryMod(team?.staff?.medStaff);
    map[String(team.id)] = {
      ...
      trainingLevel: Number(investments?.trainingLevel ?? 1),
      scoutingLevel: Number(investments?.scoutingLevel ?? 1),
      medicalSupport: Math.max(0, Math.min(0.35, (1 - medicalMod) * 0.8 + Number(staffBonuses?.recoveryDelta ?? 0))),
      continuityScore: Math.max(-0.2, Math.min(0.2, Number(staffBonuses?.moraleStabilityDelta ?? 0))),
      developmentPrecision: Math.max(-0.1, Math.min(0.25, Number(staffBonuses?.mentorDelta ?? 0) + ((Number(investments?.trainingLevel ?? 1) - 3) * 0.02))),
      staffBonuses,
    };
  }
  return map;
...
function applyWeeklyEvolution({ week, seasonId, results, metaObj }) {
  ...
  const evolution = processWeeklyEvolution({
    ...
    week,
    seasonId,
    seed: buildDeterministicSeed(`weekly_evolution_v2:${Number(metaObj?.year ?? 2025)}:${seasonId}:${week}`),
  ...
  for (const update of evolution.updates) {
    ...
    cache.updatePlayer(update.playerId, {
      ...
      wearAndTear: Number(update?.wearAndTear ?? player?.wearAndTear ?? 0),
      developmentContext: update?.developmentContext ?? player?.developmentContext ?? null,
    });
  }
...
async function handleSaveSlot({ slotKey }, id) {
  ...
  try {
    ...
    _saveIsExplicitlyLoaded = true;
    post(toUI.SAVED, { kind: 'slot', slotKey }, id);
    post(toUI.STATE_UPDATE, buildViewState(), id);
  } catch (err) {
...
async function handleSaveNow(payload, id) {
  ...
  try {
    ...
    await flushDirty();
    post(toUI.SAVED, { kind: 'now', slotKey: getActiveLeagueId() ?? null }, id);
  } catch (err) {
...
async function handleGetRoster({ teamId }, id) {
  ...
  const players = cache.getPlayersByTeam(numId).map(p => {
      ...
      return {
        ...
        weeklyTrainingBoost: Number(p?.weeklyTrainingBoost ?? 0),
        wearAndTear:      Number(p?.wearAndTear ?? 0),
        growthHistory:    Array.isArray(p?.growthHistory) ? p.growthHistory.slice(-8) : [],
        developmentContext: p?.developmentContext ?? null,
        status:           p.status ?? 'active',
...
async function handleConductDrill({ teamId, intensity, drillType, positionGroups }, id) {
    ...
    const drillSeed = buildDeterministicSeed(`drill:${meta?.currentSeasonId ?? 1}:${meta?.currentWeek ?? 1}:${numId}:${intensity ?? 'normal'}:${drillType ?? 'technique'}:${(positionGroups ?? []).join(',')}`);
    const deterministicUnit = (...parts) => buildDeterministicSeed([drillSeed, ...parts].join('|')) / 4294967295;

    // Intensity → development multiplier and boost magnitude
    const INTENSITY_CFG = {
        light:  { devMult: 0.7, maxBoost: 1, injRisk: 0.004, wear: 0.5 },
        normal: { devMult: 1.0, maxBoost: 1, injRisk: 0.009, wear: 1 },
        hard:   { devMult: 1.3, maxBoost: 2, injRisk: 0.017, wear: 1.8 },
    };
    ...
    for (const p of players) {
        ...
        const potentialGap = Math.max(0, Number(p?.potential ?? p?.ovr ?? 70) - Number(p?.ovr ?? 70));
        const focused = focusSet.size === 0 || Array.from(focusSet).some((group) => (POS_GROUP_MAP[group] || [group]).includes(p.pos));
        const chance = Math.max(0.18, Math.min(0.82, 0.28 + cfg.devMult * 0.16 + potentialGap * 0.012 + (focused ? 0.08 : 0)));
        const roll = deterministicUnit(p.id, 'boost');
        if (roll < chance) {
            const boost = Math.min(3, cfg.maxBoost + drillBonus + (focused ? 1 : 0));
            ...
            const wearAndTear = Math.max(0, Math.min(100, Number(p?.wearAndTear ?? 0) + cfg.wear - Math.max(0, (1 - medicalMod) * 3)));
            cache.updatePlayer(p.id, { weeklyTrainingBoost: p.weeklyTrainingBoost, wearAndTear });
            dirtyPlayers.push(p.id);
        }
        // Deterministic drill injury pressure, softened by medical support.
        const injuryRisk = Math.max(0, cfg.injRisk * medicalMod * (focused && intensity === 'hard' ? 1.2 : 1));
        if (injuryRisk > 0 && deterministicUnit(p.id, 'injury') < injuryRisk && !p.injured) {
            p.injured = true;
            p.injuryWeeksRemaining = 1;
            cache.updatePlayer(p.id, {
              injured: true,
              injuryWeeksRemaining: 1,
              wearAndTear: Math.max(0, Math.min(100, Number(p?.wearAndTear ?? 0) + 3)),
            });
            dirtyPlayers.push(p.id);
        }
    }
...
async function handleAdvanceOffseason(payload, id) {
  ...
  const developmentModel = normalizeWeeklyDevelopmentMeta(meta);
  const offseasonKey = `${meta?.currentSeasonId ?? meta?.year ?? 2025}:${meta.phase}:v2`;
  if (developmentModel.lastOffseasonEvolutionKey === offseasonKey) {
    post(toUI.NOTIFICATION, { level: 'info', message: 'Offseason development has already been applied for this transition.' }, id);
    post(toUI.STATE_UPDATE, buildViewState());
    return;
  }
  ...
  // ── Step 2: Deterministic offseason evolution ────────────────────────────
  const allPlayers = cache.getAllPlayers();
  const schemeFitByPlayerId = new Map();
  for (const team of allTeams) {
    const roster = allPlayers.filter((p) => Number(p?.teamId) === Number(team?.id));
    const hc = ensureTeamStaff(team, { year: Number(meta?.year ?? 2025) })?.headCoach;
    const fits = computeTeamSchemeFits(
      roster,
      team?.strategies?.offSchemeId || hc?.offScheme || 'Balanced',
      team?.strategies?.defSchemeId || hc?.defScheme || '4-3',
    );
    for (const fit of fits) schemeFitByPlayerId.set(String(fit.playerId), Number(fit.schemeFit ?? 50));
  }
  const evolutionPlayers = allPlayers.map((player) => ({
    ...player,
    schemeFit: schemeFitByPlayerId.get(String(player.id)) ?? Number(player?.schemeFit ?? 50),
  }));
  const offseasonEvolution = processOffseasonEvolution({
    players: evolutionPlayers,
    seasonId: Number(meta?.currentSeasonId ?? meta?.year ?? 2025),
    year: Number(meta?.year ?? 2025),
    seed: buildDeterministicSeed(`offseason_evolution_v2:${Number(meta?.year ?? 2025)}:${meta.phase}`),
    teamFocusByTeamId: buildTeamDevelopmentFocusMap(meta),
  });

  for (const update of offseasonEvolution.updates) {
    const player = cache.getPlayer(update.playerId);
    if (!player || player.status === 'draft_eligible' || player.status === 'retired') continue;
    const growthHistory = Array.isArray(player?.growthHistory) ? player.growthHistory : [];
    cache.updatePlayer(update.playerId, {
      ovr: update.ovr,
      potential: update.potential,
      progressionDelta: update.progressionDelta ?? null,
      attributesV2: update.attributesV2,
      attributeXp: update.attributeXp,
      wearAndTear: Number(update?.wearAndTear ?? player?.wearAndTear ?? 0),
      developmentContext: update?.developmentContext ?? player?.developmentContext ?? null,
      growthHistory: [...growthHistory.slice(-23), update.growthHistoryEntry],
      personalityProfile: player.personalityProfile ?? ensurePersonalityProfile(player),
      developmentHistory: player.developmentHistory ?? [],
    });
  }

  const topGainers = offseasonEvolution.gainers.slice(0, 5);
  const topRegressors = offseasonEvolution.regressors.slice(0, 5);
  ...
  if (topGainers.length > 0) {
    const names = topGainers
      .map(p => `${p.name} (${p.pos}, ${p.tag === 'breakout' ? 'Breakout ' : ''}+${p.delta} OVR)`)
      .join(', ');
    await NewsEngine.logNews(
      'PROGRESSION',
  ...
  if (topRegressors.length > 0) {
    const names = topRegressors
      .map(p => `${p.name} (${p.pos}, ${p.tag === 'decline' ? 'Decline ' : ''}${p.delta} OVR)`)
      .join(', ');
    await NewsEngine.logNews(
      'PROGRESSION',
  ...
  cache.setMeta({
    developmentModel: { ...developmentModel, lastOffseasonEvolutionKey: offseasonKey },
