...
function AppContent() {
  ...
  const [newSlotBootstrapPhase, setNewSlotBootstrapPhase] = useState(NEW_SLOT_BOOTSTRAP_PHASES.IDLE);
  const archiveMigrationRef = useRef(null);
  const newSlotSaveRequestTickRef = useRef(0);
  const leagueReady = hasMinimumPlayableLeague(league);
  const bootstrapSummary = summarizeBootstrapState(league);
  const loadFlowReady = initFlow?.mode === 'load'
    && leagueReady
    && initFlow?.slotKey
    && league?.activeLeagueId === initFlow.slotKey;
  const isAuthoritativeInitGateVisible = shouldShowAuthoritativeInitGate({
    league,
    initFlowMode: initFlow?.mode,
    initFlowActive: initFlow?.active,
    pendingNewSlot,
    loadReady: loadFlowReady,
    bootstrapPhase: newSlotBootstrapPhase,
  });
  const canPersistSlotState = canPersistActiveSlot({
    league,
    activeSlot,
    pendingNewSlot,
    bootstrapPhase: newSlotBootstrapPhase,
  ...
  const resetSaveSlotFlow = useCallback(() => {
    setPendingNewSlot(null);
    setActiveSlot(null);
    setActiveView('saves');
    setInitFlow(null);
    setNewSlotBootstrapPhase(NEW_SLOT_BOOTSTRAP_PHASES.IDLE);
    newSlotSaveRequestTickRef.current = saveTick;
  }, [saveTick]);

  const persistActiveSlot = useCallback(() => {
    if (!canPersistSlotState || !activeSlot) return;
    actions.saveSlot(activeSlot);
  }, [actions, activeSlot, canPersistSlotState]);
  ...
  useEffect(() => {
    const handler = () => { persistActiveSlot(); };
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === ' ' || e.key === 'Enter') {
        ...
        if (!busy && canPersistSlotState) persistActiveSlot();
      } else if (e.key === '?') {
  ...
  }, [league, busy, canPersistSlotState, postGameResult, promptUserGame, userGameLogs, handleAdvanceWeek, persistActiveSlot]);
  ...
  }, [canPersistSlotState, league, activeSlot]);
  ...
  useEffect(() => {
    if (!shouldStartNewSlotInitialSave({
      league,
      pendingNewSlot,
      bootstrapPhase: newSlotBootstrapPhase,
    })) return;
    newSlotSaveRequestTickRef.current = saveTick;
    setNewSlotBootstrapPhase(NEW_SLOT_BOOTSTRAP_PHASES.SAVING_SLOT);
    actions.saveSlot(pendingNewSlot);
  }, [league, pendingNewSlot, newSlotBootstrapPhase, saveTick, actions]);

  useEffect(() => {
    if (!shouldFinalizeNewSlotBootstrap({
      pendingNewSlot,
      bootstrapPhase: newSlotBootstrapPhase,
      saveEvent: lastSaveEvent,
    })) return;
    if (saveTick <= newSlotSaveRequestTickRef.current) return;
    setPendingNewSlot(null);
    setInitFlow(null);
    setNewSlotBootstrapPhase(NEW_SLOT_BOOTSTRAP_PHASES.IDLE);
  }, [pendingNewSlot, newSlotBootstrapPhase, lastSaveEvent, saveTick]);

  useEffect(() => {
    if (!initFlow?.active) return;
    const loadReady = initFlow.mode === 'load'
      && hasMinimumPlayableLeague(league)
      && initFlow?.slotKey
      && league?.activeLeagueId === initFlow.slotKey;
    const newReady = initFlow.mode === 'new'
      && pendingNewSlot
      && newSlotBootstrapPhase === NEW_SLOT_BOOTSTRAP_PHASES.IDLE
      && hasMinimumPlayableLeague(league);
    if (loadReady || newReady) {
      setInitFlow(null);
      return;
    }
  ...
  }, [initFlow, league, pendingNewSlot, newSlotBootstrapPhase]);
  ...
  const utilityActions = useMemo(() => {
    ...
    items.push(
      { label: 'Save Game', onClick: () => persistActiveSlot(), disabled: !canPersistSlotState || busy || isBatchSimBlocking },
      { label: 'Save Slots', onClick: () => resetSaveSlotFlow(), disabled: busy || isBatchSimBlocking },
      { label: 'Reset Franchise', onClick: handleReset, disabled: busy || isBatchSimBlocking, danger: true },
    );

    return items;
  }, [safePhase, canUseTopActions, canPersistSlotState, busy, isBatchSimBlocking, handleReset, handleSimToPhase, persistActiveSlot, resetSaveSlotFlow]);
  ...
  if (isAuthoritativeInitGateVisible) {
    const initMessage = initFlow?.mode === 'load'
      ? `Loading playable league state… ${bootstrapSummary.reasons[0] ?? 'Restoring franchise data.'}`
      : newSlotBootstrapPhase === NEW_SLOT_BOOTSTRAP_PHASES.SAVING_SLOT
        ? 'Finalizing franchise save slot… Persisting opening franchise state.'
        : `Loading playable league state… ${bootstrapSummary.reasons[0] ?? 'Preparing franchise data.'}`;

    return (
      <div className="app-loading">
        <div className="app-loading-spinner" />
        <p className="app-loading-text">{initMessage}</p>
        {initFlow?.timedOut && (
            ...
            <div style={{ display: 'inline-flex', gap: 8, marginLeft: 10 }}>
              ...
              {initFlow.mode === 'new' && (
                <button
                  ...
                  onClick={() => {
                    setInitFlow((prev) => prev ? { ...prev, active: false, timedOut: false, message: '' } : prev);
                    setNewSlotBootstrapPhase(NEW_SLOT_BOOTSTRAP_PHASES.IDLE);
              ...
              <button className="btn app-banner-btn" onClick={resetSaveSlotFlow}>
                Back to Slots
              </button>
            </div>
          </div>
          ...
          <div className="view fade-in" key="new_league">
            ...
            <NewLeagueSetup
              ...
              onStartCreate={() => {
                ...
                setNewSlotBootstrapPhase(NEW_SLOT_BOOTSTRAP_PHASES.AWAITING_PLAYABLE);
              }}
              onCancel={resetSaveSlotFlow}
            />
          </div>
      </ErrorBoundary>
    );
  ...
  const isNewFranchiseBootstrapping = false;
  ...
  return (
    <div className={`app-shell ${isPostseason ? 'postseason' : ''} ${themeClass}`} key="league_dashboard" data-testid="app-shell-ready">
      ...
      {!bootstrapSummary.ready && !initFlow?.active && !pendingNewSlot && (
