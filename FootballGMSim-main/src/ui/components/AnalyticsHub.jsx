...
export default function AnalyticsHub({ league, actions, onPlayerSelect = null, onTeamSelect = null, onNavigate = null }) {
  ...
  return <div style={{ padding: "var(--space-4)", display: "flex", flexDirection: "column", gap: 16 }}>
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><h2 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 900 }}>📊 Analytics Hub</h2><div style={{ fontSize: "0.72rem", color: "var(--text-muted)", marginTop: 3 }}>{league?.year ?? ""} Season · Week {league?.week ?? 1}</div></div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{[["dashboard", "Dashboard"], ["players", "Players"], ["teams", "Team Analytics"], ["office", "League Office"]].map(([v, label]) => <button key={v} onClick={() => setView(v)} style={pillStyle(view === v)}>{label}</button>)}</div></div>
    ...
    {view === "dashboard" ? <AnalyticsDashboard league={league} actions={actions} /> : null}
    ...
    </> : view === "office" ? <TransactionFeed transactions={transactions} filterType={txType} setFilterType={setTxType} teamFilter={txTeamFilter} setTeamFilter={setTxTeamFilter} search={txSearch} setSearch={setTxSearch} teams={league?.teams ?? []} onPlayerSelect={onPlayerSelect} onTeamSelect={onTeamSelect} /> : view === "players" ? <>
