    ...
    <div className="app-screen-stack">
      ...
      {subtab === 'Overview' && (
        <div className="app-screen-stack">
          ...
          <SectionCard title="Development Snapshot" subtitle="Recent movement tied to usage and training." variant="compact">
            <div className="app-row-stack">
              <CompactListRow
                title={developmentSummary.rising[0]?.name ?? 'No clear riser'}
                subtitle={developmentSummary.rising[0] ? `${developmentSummary.rising[0]?.pos} · trending up through live reps` : 'No player has separated from the pack yet.'}
                meta={<StatusChip label="Riser" tone="ok" />}
              />
              <CompactListRow
                title={developmentSummary.slipping[0]?.name ?? 'No clear faller'}
                subtitle={developmentSummary.slipping[0] ? `${developmentSummary.slipping[0]?.pos} · maintenance/regression pressure` : 'No player is carrying meaningful regression pressure.'}
                meta={<StatusChip label="Faller" tone={developmentSummary.slipping[0] ? 'warning' : 'neutral'} />}
              />
              <CompactListRow
                title={developmentSummary.upsideUsage[0]?.name ?? 'No young snap earner'}
                subtitle={developmentSummary.upsideUsage[0] ? `${developmentSummary.upsideUsage[0]?.pos} · young player converting live snaps into growth` : 'No young player is earning a sustained development runway yet.'}
                meta={<StatusChip label="Upside Snaps" tone={developmentSummary.upsideUsage[0] ? 'team' : 'neutral'} />}
              />
              <CompactListRow
                title={developmentSummary.stalledByUsage[0]?.name ?? developmentSummary.blocked[0]?.name ?? 'No stalled prospect'}
                subtitle={developmentSummary.stalledByUsage[0] || developmentSummary.blocked[0] ? `${(developmentSummary.stalledByUsage[0] ?? developmentSummary.blocked[0])?.pos} · reps are limiting growth direction` : 'No prospect is currently stalled by lack of weekly usage.'}
                meta={<StatusChip label="Usage Pressure" tone={developmentSummary.stalledByUsage[0] || developmentSummary.blocked[0] ? 'warning' : 'neutral'} />}
              />
            </div>
          </SectionCard>
        ...
        <div className="app-screen-stack">
          <SectionCard title="Development board" subtitle="Risers, fallers, and blocked prospects." variant="compact">
            <StatStrip items={[
              ...
              { label: 'Upside Snaps', value: `${developmentSummary.upsideUsage.length}`, tone: developmentSummary.upsideUsage.length ? 'team' : 'neutral' },
