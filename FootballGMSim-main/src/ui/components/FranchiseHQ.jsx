...
export default function FranchiseHQ({ league, onNavigate, onOpenBoxScore, onAdvanceWeek, busy, simulating }) {
  ...
  const developmentSummary = summarizeRosterDevelopment(Array.isArray(team?.roster) ? team.roster : [], new Map());
  ...
  return (
    <div className="app-screen-stack franchise-hq">
      ...
      <SectionCard title="Development Pressure" subtitle="Signals that should affect lineup and roster planning." variant="compact">
        <div className="app-priority-rail">
          <CompactInsightCard
            title={developmentSummary.rising[0]?.name ?? "No breakout candidate"}
            subtitle={developmentSummary.rising[0] ? `${developmentSummary.rising[0]?.pos} is converting current role into upward movement.` : "No player has built a strong breakout case yet."}
            tone={developmentSummary.rising[0] ? "ok" : "info"}
            ctaLabel="Open Team"
          ...
          <CompactInsightCard
            ...
            subtitle={developmentSummary.stalledByUsage[0] || developmentSummary.blocked[0] ? `${(developmentSummary.stalledByUsage[0] ?? developmentSummary.blocked[0])?.pos} needs a cleaner path to snaps.` : "No young player is being materially stalled by usage right now."}
            tone={developmentSummary.stalledByUsage[0] || developmentSummary.blocked[0] ? "warning" : "info"}
            ctaLabel="Depth Chart"
            onCta={() => onNavigate?.("Team:Roster / Depth")}
          />
          <CompactInsightCard
            title={developmentSummary.slipping[0]?.name ?? "No veteran decline flag"}
            subtitle={developmentSummary.slipping[0] ? `${developmentSummary.slipping[0]?.pos} is carrying maintenance/regression pressure.` : "No veteran is showing a major decline warning."}
            tone={developmentSummary.slipping[0] ? "warning" : "info"}
            ctaLabel="Roster"
            onCta={() => onNavigate?.("Team:Development")}
          />
        </div>
