...
const recentGrowthEntries = (player, count = 4) => Array.isArray(player?.growthHistory) ? player.growthHistory.slice(-count) : [];
const recentGrowthDelta = (player, count = 4) => recentGrowthEntries(player, count).reduce((sum, entry) => sum + safeNumber(entry?.totalDelta, 0), 0);
const recentUsage = (player, count = 4) => {
  const entries = recentGrowthEntries(player, count);
  if (!entries.length) return safeNumber(player?.developmentContext?.recentUsage, 0);
  return entries.reduce((sum, entry) => sum + safeNumber(entry?.usage, 0), 0) / entries.length;
};

export function classifyDevelopmentTrend(player) {
  const age = safeNumber(player?.age, 30);
  const delta = recentGrowthDelta(player, 4) || safeNumber(player?.progressionDelta, 0);
  ...
  const usage = recentUsage(player, 4);
  const wear = safeNumber(player?.wearAndTear ?? player?.developmentContext?.wearAndTear, 0);

  if (delta >= 2 && age <= 26 && usage >= 0.6) {
  ...
  if ((delta <= -2 && age >= 29) || (age >= 31 && wear >= 28)) {
  ...
  if (age <= 24 && delta <= 0 && upsideGap >= 6) {
    return {
      ...
      label: injuryWeeks > 0 ? 'Slowed prospect' : usage < 0.35 ? 'Stagnating prospect' : 'Holding pattern',
      tone: 'warn',
      icon: injuryWeeks > 0 ? '🩹' : '⏳',
    };
  }
...
export function getPlayerReadiness(player) {
  ...
  if ((depthOrder >= 3 || usage < 0.35) && age <= 25 && ovr >= 68) {
    return {
      key: 'blocked',
...
export function summarizeRosterDevelopment(players = [], moraleById = new Map()) {
  ...
  for (const player of list) {
    ...
    if (age <= 24 && usage >= 0.6) upsideUsage.push(player);
    if (age <= 25 && usage < 0.35 && safeNumber(player?.potential, safeNumber(player?.ovr, 0)) - safeNumber(player?.ovr, 0) >= 6) stalledByUsage.push(player);
  ...
  return {
    ...
    upsideUsage: upsideUsage.sort(sortByDelta),
    stalledByUsage: stalledByUsage.sort((a, b) => safeNumber(a?.age, 99) - safeNumber(b?.age, 99)),
