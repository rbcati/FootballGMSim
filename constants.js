...
// Ensure window exists for Node.js environment compatibility (optional, but good for testing)
...
// Note: Since this is now an ES Module (deferred), window.EXPANDED_* should be available
// even if expanded-names.js is a sync script loaded before.
export const FIRST_NAMES = window.EXPANDED_FIRST_NAMES || ['James', 'Michael', 'John', 'Robert', 'David', 'William', 'Richard', 'Joseph', 'Thomas', 'Christopher'];
export const LAST_NAMES = window.EXPANDED_LAST_NAMES || ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez'];
...
// --- MERGE WITH EXISTING GLOBAL CONSTANTS ---
// Critical: Merge properties from window.Constants (which might have been populated by sync scripts like teams.js)
if (window.Constants) {
  // We use assign to merge, so we keep properties like TEAMS_REAL
  Object.assign(Constants, window.Constants);
}

// Update the global reference to point to our complete object
window.Constants = Constants;
window.constants = Constants;

// Make individual arrays globally available (for compatibility)
// We only set them if they aren't already set (teams.js might set them?)
// But expanded-names sets them.
if (!window.FIRST_NAMES) window.FIRST_NAMES = FIRST_NAMES;
if (!window.LAST_NAMES) window.LAST_NAMES = LAST_NAMES;
